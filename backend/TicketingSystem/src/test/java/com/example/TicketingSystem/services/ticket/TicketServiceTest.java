package com.example.TicketingSystem.services.ticket;
...
class TicketServiceTest {
    ...
    void testSendRemindersToAssignees_ShortDue() {
        ...
        ticket.setStatus("Open");
        ticket.setDueDate(LocalDate.now().plusDays(1)); // Short due date
        ...
        department.setIsActive(true); // or use constructor if available
        when(ticketDepartmentRepository.findByEmailId("agent@example.com"))
    ...
    void testSendRemindersToAssignees_Overdue() {
        ...
        ticket.setDueDate(LocalDate.now().minusDays(1)); // Overdue
        ticket.setCreatedDate(LocalDateTime.now().minusDays(5));
        ticket.setUpdatedDate(LocalDateTime.now());

        TicketDepartment user = new TicketDepartment();
        user.setEmailId("agent@example.com");
        user.setIsActive(true);

        when(ticketRepository.findAll()).thenReturn(List.of(ticket));
        when(ticketDepartmentRepository.findByEmailId("agent@example.com")).thenReturn(Optional.of(user));
